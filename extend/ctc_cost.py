...
Recurrent Relation:
    ...
    At any time, one could feed in from the
        ...
        1) prev label (unless of course you are the first)
...
class CTC(object):
    ...
    # distribute the probability of each class to predicted label sequence
    ...
    def recurrence_relation(y, y_mask, blank_symbol):
        n_y = y.shape[0]
        blanks = tensor.zeros((2, y.shape[1])) + blank_symbol   # append two blank symbols to the end
        ybb = tensor.concatenate((y, blanks), axis=0).T
        sec_diag = (tensor.neq(ybb[:, :-2], ybb[:, 2:]) *
                    tensor.eq(ybb[:, 1:-1], blank_symbol) *
                    y_mask.T)   # mark if l(s)=blank or l(s-2)=l(s)

        # r1: LxL   current label
        # r2: LxL   previous label
        # r3: LxLxB
    ...
    @classmethod
    def apply(cls, y, y_hat, scale='log_scale'):
        y_hat_mask=tensor.ones((y_hat.shape[0], y_hat.shape[1]))
        y_mask = tensor.ones_like(y)
