...
def read_Cdata(path = "./avletters/Lips/", test_split = 0.2, data_split = 1, seed=113, channel = 4, shulffle=True):
...
def read_data_sequence(path = "./avletters/Lips/", test_split=0.2, data_split = 1, sampling_step=3, seed = 113):
    ...
    height = 0
    width = 0
    sam_total_num = 0       # total slice number
    label = []
    max_len = 0
    ...
    data = np.zeros((sam_total_num, int(max_len/sampling_step), 1, height, width))
    ...
    for item in filespath:
        ...
        for step_i in range(sampling_step):
            for slice_ind in range(int(seq_length/sampling_step)):
                slice = temp_data[:, slice_ind*sampling_step+step_i]
                slice = slice.reshape((width, height))
                data[sample_index, slice_ind, 0, :, :] = np.transpose(slice)
